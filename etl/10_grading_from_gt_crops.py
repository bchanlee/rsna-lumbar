...
study_ids = []
levels = []
image_dir = "../data/train_crops_gt_coords"
...
for study_id, fold in tqdm(study_id_fold_dict.items(), total=len(study_id_fold_dict)):
    ...
    for each_series in series:
        ...
        study_ids.append(study_id) # assume all sag_t2
        levels.append(level) # assume all sag_t2
...
pred_df = pd.DataFrame({
    "study_id": study_ids,
    "level": levels,
    "normal_mild_pred_gt": normal_mild,
    "moderate_pred_gt": moderate,
    "severe_pred_gt": severe
})

pred_df.to_csv("../data/gt_crops_preds.csv", index=False)
